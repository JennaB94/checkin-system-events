...
async function loadInitialStateFromFirebase() {
  ...
  try {
    ...
    if (data.checkpoints) {
      state.checkpoints = Object.values(data.checkpoints);
    } else {
      state.checkpoints = [];
    }
    if (typeof renderCheckpoints === 'function') renderCheckpoints();
    ...
    if (data.checkins) {
      state.log = Object.entries(data.checkins).map(([key, value]) => ({ id: value.id || key, ...value }));
    } else {
      state.log = [];
    }
...
async function saveStateToFirebase(appState) {
  ...
  try {
    ...
    (appState.participants || []).forEach(p => { participantsObj[p.id] = p; });

    const checkpointsObj = {};
    (appState.checkpoints || []).forEach(cp => { checkpointsObj[cp.id] = cp; });

    const checkinsObj = {};
    (appState.log || []).forEach(entry => {
      const id = entry.id || database.ref('checkins').push().key;
      checkinsObj[id] = { ...entry, id };
    });

    await database.ref().update({
      event: appState.event || null,
      participants: Object.keys(participantsObj).length ? participantsObj : null,
      checkpoints: Object.keys(checkpointsObj).length ? checkpointsObj : null,
      checkins: Object.keys(checkinsObj).length ? checkinsObj : null
    });

    syncState.lastSync = new Date();
    updateLastSyncDisplay();
    console.log('✅ Full state mirrored to Firebase');
    return true;
  } catch (error) {
...
async function clearParticipantsFromFirebase() {
  if (!database) return false;
  try {
    await database.ref('participants').remove();
    console.log('✅ Participants cleared from Firebase');
    return true;
  } catch (error) {
    console.error('❌ Error clearing participants from Firebase:', error);
    return false;
  }
}

async function clearCheckpointsFromFirebase() {
  if (!database) return false;
  try {
    await database.ref('checkpoints').remove();
    console.log('✅ Checkpoints cleared from Firebase');
    return true;
  } catch (error) {
    console.error('❌ Error clearing checkpoints from Firebase:', error);
    return false;
  }
}
...
async function deleteParticipantFromFirebase(id) {
  ...
  try {
    ...
    const checkinsSnap = await database.ref('checkins').once('value');
    const updates = {};
    checkinsSnap.forEach(child => { if (child.val() && child.val().participantId === id) updates['checkins/' + child.key] = null; });
...
async function deleteCheckpointFromFirebase(id) {
  ...
  try {
    ...
    const checkinsSnap = await database.ref('checkins').once('value');
    const updates = {};
    checkinsSnap.forEach(child => { if (child.val() && child.val().checkpointId === id) updates['checkins/' + child.key] = null; });
    if (Object.keys(updates).length) await database.ref().update(updates);
...
async function saveCheckInToFirebase(checkinData) {
  try {
    ...
    syncState.isSyncing = true;
    const id = checkinData.id || database.ref('checkins').push().key;
